...
public class TaskDemolish extends Task{
    ...
    @Override
    public void finish(){
        game.addParticleEffect(new Particle(game, building.x+building.width/2, building.y+building.height/2-1, ParticleEffectType.EXPLOSION, 5));
        if(building.type==BuildingType.SKYSCRAPER){
            Skyscraper sky = (Skyscraper) building;
            sky.falling = true;
        }else{
            if(building.type.costs.length==1){
                game.replaceStructure(building, new Wreck(game, building.x, building.y, building.type.costs[0][0].count));
            }else{
                game.replaceStructure(building, new Wreck(game, building.x, building.y, building.type.costs[0][0].count+(building.type.costs[1][0].count*building.getLevel())));
            }
        }
    }
    @Override
