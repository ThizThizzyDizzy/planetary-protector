...
//    }
    ...
    protected void fakeDrawBar(float percent, float r, float g, float b){
        if(falling)return;
        super.fakeDrawBar(percent, r, g, b);
    }
    @Override
    ...
    protected Mesh generateMesh(){
        ...
        builder.quadXY(-getSize().x/2, -getSize().y/2, getSize().x/2, getSize().y/2, getSize().z, true, 0, 10/110f, 1, 1);
        for(int i = 0; i<floorCount; i++){
            builder.quadXZ(-getSize().x/2, floorHeight*i, getSize().x/2, floorHeight*(i+1), getSize().y/2, true, 0, 0, 1, 10/110f);
        }
