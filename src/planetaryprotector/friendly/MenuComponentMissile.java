...
public class MenuComponentMissile extends MenuComponentDrone{
    ...
    int power = 2500;
    int time = 0;
    public MenuComponentMissile(MenuComponentSilo silo, MenuComponentEnemy target){
        super(silo, 0);
        this.silo = silo;
        this.target = target;
    }
    ...
    public void tick(){
        ...
        if(MenuOptionsGraphics.particles>0&&!caboom){
            if(MenuOptionsGraphics.particles==1&&time%2==0||MenuOptionsGraphics.particles==2){
                Core.game.addParticleEffect(new MenuComponentParticle(x, y+(accel<0?0:height), ParticleEffectType.SMOKE));
            }
        }
        if(y>target.y&&accel<0&&!caboom){
            ...
            Core.game.addParticleEffect(new MenuComponentParticle(x, y, ParticleEffectType.EXPLOSION, Math.min(9, power/1000), target instanceof EnemyAlien?false:(target instanceof EnemyLandingParty?((EnemyLandingParty)target).landed:false)));
        }
    }
    @Override
    public void render(){
